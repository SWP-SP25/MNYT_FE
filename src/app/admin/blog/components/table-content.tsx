...
export const TableContent = () => {
  ...
  const getFilteredData = () => {
    ...
    if (!searchText) {
      return activeTab === "admin"
        ? data
        : data?.filter((blog) => blog.status !== "Draft");
    }
  ...
  const columns: TableProps<Blog>["columns"] = [
    ...
    },
    {
      ...
      filters: blogView?.data
        ? [...new Set(blogView.data.map((blog) => blog.category))].map(
  ...
  return (
    <div>
      ...
      <Table<Blog> columns={columns} dataSource={getFilteredData()} />
